...
def modify_course():
    ...
    try:
        cur.execute("UPDATE courses SET name = COALESCE(%s, name), day = COALESCE(%s, day), max_enrolled = COALESCE(%s, max_enrolled), year_req =  COALESCE(%s, year_req), hours = COALESCE(%s, hours), lecture_code = COALESCE(%s, code) WHERE crn = %s ", (name, day, max_enrolled, year_req, hours, code, crn,))
        conn.commit()
    except Exception as err:
        print(err)
        conn.rollback()
